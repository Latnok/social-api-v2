...
class RedisCache {
    constructor(url) { this.r = new ioredis_1.default(url, { lazyConnect: true }); }
    async ensure() { if (this.r.status === 'wait' || this.r.status === 'end')
    ...
    async del(key) { await this.ensure(); await this.r.del(key); }
    async delByPrefix(prefix) {
        await this.ensure();
...
exports.cache = process.env.REDIS_URL
