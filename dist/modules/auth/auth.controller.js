...
const register = async (req, res) => {
    ...
    if (existing)
        throw new error_1.HttpError(409, 'User with this email already exists');
...
const login = async (req, res) => {
    ...
    if (!email || !password)
        throw new error_1.HttpError(400, 'email and password are required');
...
const refresh = async (req, res) => {
    ...
    if (!token)
        throw new error_1.HttpError(401, 'No refresh token');
    let payload;
    try {
