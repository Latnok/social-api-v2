...
class MemoryCache implements CacheDriver {
  ...
  async get<T>(key: string): Promise<T | null> {
    ...
    if (!row) return null;
    if (row.exp < Date.now()) { this.store.delete(key); return null; }
...
class RedisCache implements CacheDriver {
  private r: IORedis;
  constructor(url: string) { this.r = new IORedis(url, { lazyConnect: true }); }
  private async ensure() { if (this.r.status === 'wait' || this.r.status === 'end') await this.r.connect(); }
  async get<T>(key: string): Promise<T | null> {
    await this.ensure(); const v = await this.r.get(key); return v ? (JSON.parse(v) as T) : null;
  }
  async set(key: string, value: Json, ttlSec: number) {
    await this.ensure(); await this.r.set(key, JSON.stringify(value), 'EX', ttlSec);
  }
  async del(key: string) { await this.ensure(); await this.r.del(key); }
  async delByPrefix(prefix: string) {
    await this.ensure();
    ...
    await new Promise<void>((resolve, reject) => {
      scan.on('data', (keys: string[]) => {
        keys.forEach((k) => { pipe.del(k); pending++; });
      });
      scan.on('end', resolve);
